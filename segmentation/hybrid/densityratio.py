...
class Densratio: 
    ...
    @property
    def SEPDiv(self):
        # return max(0, 1-np.dot(self.__phi_y, self.__weights_).mean())
        g_y=np.dot(self.__phi_y, self.__weights)
        return max(0, -0.5*g_y.T.dot(g_y)/self.__y_num_row-self.PEDiv)

    @property
    def SEPDiv_(self):
        # return max(0, 1-np.dot(self.__phi_y, self.__weights_).mean())
        g_y=np.dot(self.__phi_y, self.__weights)
        return abs(-0.5*g_y.T.dot(g_y)/self.__y_num_row-self.PEDiv)
    ...
    def _RuLSIF(self,x,y,alpha,s_sigma,s_lambda):
        ...
        phi_x=gauss_kernel_(x, self.__centers, sigma)
        # phi_y = self.__kernel(r = y, sigma = sigma)
        phi_y=gauss_kernel_(y, self.__centers, sigma)

        # print("Similarity (Kernel center and numer)")
        # print(phi_x)
        # print("Similarity (Kernel center and denom)")
        # print(phi_y)

        # H = (1.- alpha)*(np.dot(phi_y.T, phi_y) / self.__y_num_row) + alpha*(np.dot(phi_x.T, phi_x)/self.__x_num_row) #Phi* Phi
        H = np.dot(phi_y.T, phi_y) / self.__y_num_row #Phi* Phi
        ...
        # h_=np.average(phi_y,axis=0).T
        weights_ = np.linalg.solve(-lambda_*np.identity(self.__kernel_num), h).ravel()
        # weights_ = jax.ops.index_update(weights_,weights_<0,0)
...
def gauss_kernel_(r, centers, sigma):
    return np.exp(-0.5*np.square(distance_matrix(r,centers)) / (sigma**2))
