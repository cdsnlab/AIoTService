...
do
    echo "lambda: $i"
    python train.py --hidden_as_input $hidden_as_input --filter_name $filter_name --lam $i --exp_info_file $file_name --device $device --random_noise $random_noise --n_fold_cv $n_fold_cv --with_other $with_other --model $model --train_filter $train_filter --dropout_rate $dropout_rate --except_all_other_events $except_all_other_events --dataset $dataset
done
