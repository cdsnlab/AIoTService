...
do
    ...
    do
        echo "filters: $i"
        echo "kernel_size: $j"
        python train.py --filters $i --kernel_size $j --lam $lambda_list --exp_info_file $file_name --device $device --random_noise $random_noise --n_fold_cv $n_fold_cv --with_other $with_other --model $model --train_attn $train_attn --dropout_rate $dropout_rate --except_all_other_events $except_all_other_events --utilize_tr $utilize_tr --dataset $dataset
    done
done
