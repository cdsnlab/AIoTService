...
import utils
from model import EARLIEST, SEGMENTATION
...
parser.add_argument("--offset", type=int, default=10, help="The offset that is considered as ground truth boundary")
...
def loss_segmentation(model, x, tr_boundary, tr_point, length):  # shape of true_y is (B,)
    pred_tr, true_tr = model(x, tr_boundary, tr_point, length, is_train=True)  # shape of pred_y is (B * nclasses)
    CE = tf.keras.losses.BinaryCrossentropy()
    loss = CE(y_true=true_tr, y_pred=pred_tr) # Classification loss
    return loss

def grad(model, x, tr_boundary, tr_point, length):
    with tf.GradientTape() as tape:
        total_loss = loss_segmentation(model, x, tr_boundary, tr_point, length)
        # if args.model == "EARLIEST":
        # elif args.model == "basic":
    return total_loss, tape.gradient(total_loss, model.trainable_variables)

def train_step(model, x, tr_boundary, tr_point, length):
    total_loss, grads = grad(model, x, tr_boundary, tr_point, length)
    optimizer.apply_gradients(zip(grads, model.trainable_variables))
    train_loss(total_loss)
    # train_accuracy(np.reshape(true_y, [-1,1]), pred_logit)

def test_step(model, x, tr_boundary, tr_point, length):
...
if __name__ == "__main__":
    ...
    for k, (train_idx, test_idx) in enumerate(kfold.split(data.X, data.Y)):
        ...
        train_loader = Dataloader(train_idx, data.X, data.Y, data.lengths, data.event_counts, args.batch_size, 
                                  shuffle=args.shuffle, tr_points=data.tr_points, tr_boundary=data.gt_boundary)
        test_loader = Dataloader(test_idx, data.X, data.Y, data.lengths, data.event_counts, args.batch_size, 
                                 shuffle=args.shuffle, tr_points=data.tr_points, tr_boundary=data.gt_boundary)
        if args.model == "EARLIEST":
            model = EARLIEST(args)
        elif args.model == "SEGMENT":
        ...
        for epoch in tqdm(range(args.nepochs)):
            ...
            for x, true_y, length, count, tr_point, tr_boundary in train_loader:
                train_step(model, x, tr_boundary, tr_point, length)
            # end of epoch
            with train_summary_writer.as_default():
                tf.summary.scalar('train_loss', train_loss.result(), step=epoch)
            print("k{}_epoch {:03d}:  train_loss: {:.3}".format(k+1, epoch, train_loss.result()))
            
            # Run test for every 10 epochs
            if (epoch + 1) % 1 == 0:
                for x, true_y, length, count, tr_point, tr_boundary in test_loader: 
